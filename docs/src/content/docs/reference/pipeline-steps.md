...
When a step is `running` or `fixing`, AXI run objects expose an `active_steps` table. The [`axi status` CLI reference](/no-mistakes/reference/cli/#no-mistakes-axi-status) owns its fields and output semantics.
